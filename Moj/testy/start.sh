...
ALLOW_PRIVATE_LAN_CORS="${ALLOW_PRIVATE_LAN_CORS:-0}"
FRESH_START=0
AUTO_OPEN=1
...
nohup env \
  ...
  CORS_ALLOW_PRIVATE_LAN="${ALLOW_PRIVATE_LAN_CORS}" \
  PORT="$PORT" \
...
ALLOW_PRIVATE_LAN_CORS=${ALLOW_PRIVATE_LAN_CORS}
EOT

echo "[Moj/testy] OK"
echo "[Moj/testy] URL: http://127.0.0.1:${PORT}"
if [[ "${BACKEND_BIND_HOST}" == "0.0.0.0" ]]; then
  echo "[Moj/testy] Tryb LAN aktywny (BIND_HOST=0.0.0.0)"
fi
